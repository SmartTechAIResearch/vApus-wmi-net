...
namespace vApus.Monitor.Sources.Base {
    ...
    public class Entities {
        ...
        public bool Match(Entities entities, bool matchCounters) {
            ...
            if (match) {
                int size = this.subs.Count;
                var matched = new List<int>();
                for (int i = 0; i != size; i++) {
                    Entity entity = this.subs[i];
        ...
        public List<CounterInfo> GetCounterInfos(int level) {
            ...
            --level;
            for (int i = 0; i != GetSubs().Count; i++) {
                List<CounterInfo> subCounterInfos = this.subs[i].GetCounterInfos(level);
                if (this.subs[i].IsAvailable() && subCounterInfos.Count == 0)
        ...
        public int GetDeepCount() {
            ...
            for (int i = 0; i != this.subs.Count; i++)
                deepCount += this.subs[i].GetDeepCount();
